#!/usr/bin/env bash
# Build and test MSM (Pippenger) on a GCP VM with AVX512-IFMA support.
# Runs correctness tests in both fallback and AVX512 modes, then benchmarks.
# Prerequisites: gcloud CLI installed and authenticated (gcloud init / gcloud auth login).
#
# Usage: ./bench_msm_gcp.sh [--keep]
#   --keep   Don't delete the VM after testing (for debugging)

set -euo pipefail

# ── Configuration ──────────────────────────────────────────────────────────
VM_NAME="vroom-msm-bench"
ZONE="us-central1-a"
...
echo "Building and running MSM tests..."
gcloud compute ssh "$VM_NAME" --zone="$ZONE" --command="
set -e
...
echo ''
echo '=========================================='
echo '  MSM Correctness Tests (fallback mode)'
echo '=========================================='
cd ~/vroom/src && make CXX=clang++ test_msm
./test_msm

echo ''
echo '=========================================='
echo '  MSM Correctness Tests (AVX512-IFMA)'
echo '=========================================='
make CXX=clang++ test_msm_avx
./test_msm_avx

echo ''
echo '=========================================='
echo '  MSM Benchmark (AVX512-IFMA)'
echo '=========================================='
if [ -f bench_msm.cpp ]; then
    make CXX=clang++ bench_msm_avx
    ./bench_msm_avx
else
    echo 'bench_msm.cpp not found, skipping benchmarks'
fi
"

echo ""
echo "MSM tests complete."
